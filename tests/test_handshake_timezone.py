...
IANA zone ID. An abbreviation is not one, and neither is `None`: the server
falls back to its own default zone for both, silently shifting every timestamp.
"""
import struct
...
from pygridgain.connection import Connection, connection
...
def local_zone(monkeypatch):
    """ Make the client see the given zone as the zone of the machine. """
    def set_local_zone(zone_id):
        monkeypatch.setattr(connection, 'get_localzone', lambda: ZoneInfo(zone_id))

    return set_local_zone


@pytest.mark.parametrize('zone_id', ZONE_IDS)
def test_connection_reports_zone_id(local_zone, zone_id):
    local_zone(zone_id)
